...
def main():
    ...
    croper = Croper("/workspace/video-retalking/checkpoints/shape_predictor_68_face_landmarks.dat")
    ...
    if not os.path.isfile("temp/" + base_name + "_coeffs.npy") or args.exp_img is not None or args.re_preprocess:
        ...
        lm3d_std = load_lm3d("/workspace/video-retalking/checkpoints/BFM")
    ...
    if args.exp_img is not None and (".png" in args.exp_img or ".jpg" in args.exp_img):
        ...
        trans_params = np.array([float(item) for item in np.hsplit(trans_params, 5)]).astype(np.float32)
        im_exp_tensor = (
            torch.tensor(np.array(im_exp) / 255.0, dtype=torch.float32).permute(2, 0, 1).to(device).unsqueeze(0)
        )
        with torch.no_grad():
            expression = split_coeff(net_recon(im_exp_tensor))["exp"][0]
        del net_recon
    elif args.exp_img == "smile":
        expression = torch.tensor(loadmat("/workspace/video-retalking/checkpoints/expression.mat")["expression_mouth"])[
            0
        ]
    else:
    ...
    out = cv2.VideoWriter(
        "temp/{}/result.mp4".format(args.tmp_dir), cv2.VideoWriter_fourcc(*"mp4v"), fps, (frame_w, frame_h)
    )
    ...
    for i, (img_batch, mel_batch, frames, coords, img_original, f_frames) in enumerate(
        tqdm(gen, desc="[Step 6] Lip Synthesis:", total=int(np.ceil(float(len(mel_chunks)) / args.LNet_batch_size)))
    ):
