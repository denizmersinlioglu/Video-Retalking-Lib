...
from third_part.GPEN.face_morpher.facemorpher import locator
from third_part.GPEN.face_morpher.facemorpher import aligner
from third_part.GPEN.face_morpher.facemorpher import warper
from third_part.GPEN.face_morpher.facemorpher import blender
from third_part.GPEN.face_morpher.facemorpher import plotter
from third_part.GPEN.face_morpher.facemorpher import videoer
...
def verify_args(args):
    if args["--images"] is None:
        ...
        if not valid:
            print(
                "--src=%s or --dest=%s file does not exist. Double check the supplied paths"
                % (args["--src"], args["--dest"])
            )
            exit(1)
    else:
...
def main():
    ...
    morpher(
        list_imgpaths(args["--images"], args["--src"], args["--dest"]),
        int(args["--width"]),
        int(args["--height"]),
        int(args["--num"]),
        int(args["--fps"]),
