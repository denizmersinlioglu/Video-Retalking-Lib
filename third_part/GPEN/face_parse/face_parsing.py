...
import torch
import numpy as np
from third_part.GPEN.face_parse.parse_model import ParseNet
import torch.nn.functional as F

from third_part.GPEN.face_parse.model import BiSeNet
...
class FaceParse(object):
    def __init__(
        self,
        base_dir="./",
        model="ParseNet-latest",
        device="cuda",
        mask_map=[0, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 0, 0, 0, 0, 0, 0],
    ):
    ...
    def load_model(self):
        self.faceparse = ParseNet(
            self.size, self.size, 32, 64, 19, norm_type="bn", relu_type="LeakyReLU", ch_range=[32, 256]
        )
...
class FaceParse_v2(object):
    def __init__(
        self, device="cuda", mask_map=[0, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 0, 0, 0, 0, 0, 0]
    ):
